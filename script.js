...
(function () {
  ...
  function initEventListeners() {
    ...
    const gestureEnd = e => {
      ...
      try { DOM.canvasScrollContainer.releasePointerCapture(e.pointerId); } catch (_) {}
      if (gesturePointers.size < 2) {
    ...
    // Apply the default Select tool behavior immediately. The button is visually
    // active in HTML, but the interaction styles (touch-action, pointer handling,
    // cursor) also need to be initialized before the first PDF is opened.
    setTool('select');
  }
  ...
  async function renderPage(pageNum) {
    ...
    // Render above CSS resolution even on normal desktop displays. This avoids
    // soft-looking PDF text after the canvas is scaled by the browser.
    const dpr = Math.min(Math.max(deviceDpr, 2), 3);
