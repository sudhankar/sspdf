...
(function () {
  ...
  function initEventListeners() {
    ...
    let lastGestureDistance = 0, lastGestureCenter = null, gestureBusy = false;
    const isTouchPointer=e=>e.pointerType==='touch';
    const gestureStart=e=>{if(!isTouchPointer(e)||state.currentTool!=='select')return;gesturePointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(gesturePointers.size===2){const pts=[...gesturePointers.values()];lastGestureDistance=Math.hypot(pts[1].x-pts[0].x,pts[1].y-pts[0].y);lastGestureCenter={x:(pts[0].x+pts[1].x)/2,y:(pts[0].y+pts[1].y)/2};gestureBusy=false;e.preventDefault();}};
    const gestureMove=e=>{if(!isTouchPointer(e)||!gesturePointers.has(e.pointerId)||state.currentTool!=='select')return;gesturePointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(gesturePointers.size!==2)return;const pts=[...gesturePointers.values()],dist=Math.hypot(pts[1].x-pts[0].x,pts[1].y-pts[0].y),center={x:(pts[0].x+pts[1].x)/2,y:(pts[0].y+pts[1].y)/2};if(lastGestureDistance&&!gestureBusy){const ratio=dist/lastGestureDistance;if(Math.abs(ratio-1)>.015){gestureBusy=true;const next=clamp(state.zoomScale*ratio,.4,3);setZoom(next,center.x,center.y).finally(()=>{gestureBusy=false;});lastGestureDistance=dist;}}if(lastGestureCenter){DOM.canvasScrollContainer.scrollLeft-=center.x-lastGestureCenter.x;DOM.canvasScrollContainer.scrollTop-=center.y-lastGestureCenter.y;}lastGestureCenter=center;e.preventDefault();};
    const gestureEnd=e=>{if(!isTouchPointer(e))return;gesturePointers.delete(e.pointerId);if(gesturePointers.size<2){lastGestureDistance=0;lastGestureCenter=null;gestureBusy=false;}};
    DOM.canvasScrollContainer.addEventListener('pointerdown',gestureStart,{passive:false});DOM.canvasScrollContainer.addEventListener('pointermove',gestureMove,{passive:false});DOM.canvasScrollContainer.addEventListener('pointerup',gestureEnd);DOM.canvasScrollContainer.addEventListener('pointercancel',gestureEnd);
