...
(function () {
  ...
  async function applyCropPdf() {
    ...
    const vals={top:Number(document.getElementById('crop-pdf-top').value)||0,right:Number(document.getElementById('crop-pdf-right').value)||0,bottom:Number(document.getElementById('crop-pdf-bottom').value)||0,left:Number(document.getElementById('crop-pdf-left').value)||0};
    ...
    try {
      for(const page of pages){
        const box=page.getMediaBox ? page.getMediaBox() : { x:0, y:0, width:page.getWidth(), height:page.getHeight() };
        const left=Math.max(0,Number(box.x)||0)+vals.left;
        const bottom=Math.max(0,Number(box.y)||0)+vals.bottom;
        const maxW=Math.max(20,(Number(box.width)||page.getWidth())-vals.left-vals.right);
        const maxH=Math.max(20,(Number(box.height)||page.getHeight())-vals.top-vals.bottom);
        // Set both MediaBox and CropBox. This makes the crop visible consistently
        // in PDF.js viewers and in downloaded PDFs, including mobile browsers.
        page.setMediaBox(left,bottom,maxW,maxH);
        page.setCropBox(left,bottom,maxW,maxH);
      }
      state.pdfBytes=await state.pdfLibDoc.save(); state.pdfDoc=await pdfjsLib.getDocument({data:new Uint8Array(state.pdfBytes.slice(0))}).promise; state.totalPages=state.pdfDoc.numPages;
      await refreshPdfViewer(); saveStateToHistory(); closeModal('modal-crop-pdf'); showToast('PDF crop applied','success');
  ...
  async function previewEnhancement(){
    ...
    try{
      ...
      // Fit the entire page inside the preview area. Do not use a fixed tiny
      // scale because A4/Letter pages then looked like a small thumbnail.
      const availableW=Math.max(220,(wrap?.clientWidth||760)-16);
      const availableH=Math.max(260,(wrap?.clientHeight||520)-16);
      const fitScale=Math.max(.2,Math.min(2.0,availableW/base.width,availableH/base.height));
