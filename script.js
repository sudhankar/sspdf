...
(function () {
  ...
  function initEventListeners() {
    ...
    // Hand-tool drag scrolling
    DOM.canvasScrollContainer.addEventListener('mousedown', e => {
      if (state.currentTool !== 'pan') return;
      state.panState = { x: e.clientX, y: e.clientY, left: DOM.canvasScrollContainer.scrollLeft, top: DOM.canvasScrollContainer.scrollTop };
  ...
  async function renderPage(pageNum) {
    ...
    // PDF.js already honors the page's native /Rotate value.
    const viewport = page.getViewport({ scale: state.zoomScale, rotation: 0 });
  ...
  function addResizeHandle(el) {
    ...
    handle.addEventListener('mousedown', e => {
      ...
      const move = ev => {
        ...
        const img = el.querySelector('img'); img.style.width = '100%'; img.style.height = '100%'; img.style.maxWidth = 'none';
      };
      const up = () => { document.removeEventListener('mousemove', move); document.removeEventListener('mouseup', up); saveStateToHistory(); };
