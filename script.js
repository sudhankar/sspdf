...
(function () {
  ...
  async function applyCropPdf() {
    ...
    try {
      for(const page of pages){
        const box=page.getCropBox ? page.getCropBox() : (page.getMediaBox ? page.getMediaBox() : {x:0,y:0,width:page.getWidth(),height:page.getHeight()});
        const boxX=Number(box.x)||0, boxY=Number(box.y)||0, boxW=Number(box.width)||page.getWidth(), boxH=Number(box.height)||page.getHeight();
        ...
        // Set both boxes. CropBox clips the visible content and MediaBox makes
        // the cropped page size persist reliably in downloaded/mobile viewers.
  ...
  function openCropImage() {
    ...
    const preview=document.getElementById('crop-image-preview'); if(preview){preview.innerHTML=''; const p=img.cloneNode(true); p.style.maxWidth='100%'; p.style.maxHeight='240px'; preview.appendChild(p);}
    ['top','right','bottom','left'].forEach(k=>{const e=document.getElementById('crop-img-'+k);if(e)e.value='0';}); openModal('modal-crop-image');
  }
  async function applyCropImage() {
    ...
    const t=Number(document.getElementById('crop-img-top').value)||0,r=Number(document.getElementById('crop-img-right').value)||0,b=Number(document.getElementById('crop-img-bottom').value)||0,l=Number(document.getElementById('crop-img-left').value)||0;
    const src=new Image(); src.src=img.src; await new Promise((res,rej)=>{src.onload=res;src.onerror=rej;});
    const sx=Math.floor(src.naturalWidth*l/100), sy=Math.floor(src.naturalHeight*t/100), sw=Math.max(1,Math.floor(src.naturalWidth*(1-(l+r)/100))), sh=Math.max(1,Math.floor(src.naturalHeight*(1-(t+b)/100)));
    const c=document.createElement('canvas'); c.width=sw; c.height=sh; c.getContext('2d').drawImage(src,sx,sy,sw,sh,0,0,sw,sh); img.src=c.toDataURL('image/png'); img.style.width='100%'; img.style.height='100%'; img.style.maxWidth='none';
    const ratio=sh/sw, w=el.offsetWidth||200, h=w*ratio; el.style.height=`${h}px`; el.dataset.pdfH=String(h/state.zoomScale); closeModal('modal-crop-image'); saveStateToHistory(); showToast('Image cropped','success');
  }
