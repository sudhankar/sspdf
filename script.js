  ...
  async function checkRecovery(){
    try{
      ...
      req.onsuccess=async()=>{
        ...
        if(!window.confirm('Unsaved SSPDF work was found. Restore it?')){clearRecovery();return;}
        try{
          const bytes=rec.pdfBytes instanceof Uint8Array?rec.pdfBytes:new Uint8Array(rec.pdfBytes);
          state.pdfBytes=bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength);
          state.fileName=rec.fileName||'Recovered.pdf';
          ...
          setTool('select'); await renderThumbnails(); await renderPage(state.currentPage); state.history=[];state.redoStack=[];saveStateToHistory();
          showToast('Unsaved work restored','success');
