...
(function () {
  ...
  function applyInlineStyleToSelection(styles) {
    const t = getEditingTextElement();
    if (!t) return false;
    // First prefer the current browser selection; if focus moved to a toolbar
    // control, restore the last selection saved by selectionchange/pointerdown.
  ...
  async function previewEnhancement(){if(!state.pdfDoc)return;const canvas=document.getElementById('enhance-preview');try{const page=await state.pdfDoc.getPage(state.currentPage),vp=page.getViewport({scale:Math.min(1.2,1000/page.getViewport({scale:1}).width)});canvas.width=Math.ceil(vp.width);canvas.height=Math.ceil(vp.height);await page.render({canvasContext:canvas.getContext('2d',{alpha:false}),viewport:vp}).promise;applyCanvasEnhancement(canvas.getContext('2d'),canvas.width,canvas.height,enhancementParams());}catch(err){console.warn(err);}}
